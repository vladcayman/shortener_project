# 🔗 Сервис коротких ссылок (URL Shortener)

Простой, удобный и расширяемый сервис для сокращения длинных URL.  
Поддерживает публичное сокращение ссылок, создание приватных ссылок для авторизованных пользователей, статистику кликов и минимальный фронтенд.

Проект выполнен на **Django + Django REST Framework** с использованием **Docker** и базой данных **PostgreSQL**.

---

## 🚀 Возможности

- ✂️ Сокращение длинных ссылок
- 👤 Создание приватных ссылок для авторизованных пользователей
- 📊 Статистика кликов (дата, время, устройство, ОС, браузер, IP)  
- 🏷 Работа с категориями и тегами
- ⚙️ Админ-панель Django для управления ссылками и пользователями  
- 🌐 Мини-фронтенд на HTML/CSS/JS  
- 🐳 Полностью Docker-изолированная инфраструктура (web + db)

---

## 🛠 Технологии

- Python 3.13
- Django 5.2.8
- Django REST Framework
- JWT-авторизация (`djangorestframework-simplejwt`)
- drf-spectacular (Swagger / OpenAPI)
- PostgreSQL 15
- Docker + Docker Compose
- HTML + JS (Frontend для формы)

---

## 📂 Структура проекта
```
shortener_project/
│
├── shortener/                          # Приложение Django
│   ├── migrations/                     # Миграции БД
│   ├── templates/                      # HTML-шаблоны (Frontend)
│   │   ├── index.html                  # Форма сокращения ссылок
│   │   └── my_links.html               # Личный кабинет (список ссылок)
│   ├── __init__.py
│   ├── admin.py                        # Регистрация моделей в админке
│   ├── apps.py
│   ├── filters.py                      # Поиск / фильтрация ссылок
│   ├── models.py                       # Модели Link, Click, Category, Tag
│   ├── permissions.py                  # Ограничения прав доступа
│   ├── serializers.py                  # DRF сериализаторы
│   ├── tests.py                        # Тесты
│   ├── urls.py                         # URL-маршруты приложения
│   ├── utils.py                        # Вспомогательные функции
│   └── views.py                        # Основная логика API/Frontend
│
├── shortener_project/                  # Основной модуль Django
│   ├── __init__.py
│   ├── asgi.py
│   ├── settings.py                     # Настройки Django
│   ├── urls.py                         # Маршрутизация проекта
│   └── wsgi.py
│
├── .dockerignore                       # Исключения для Docker контекста
├── .gitignore                          # Исключения для Git
│
├── docker-compose.yml                  # Docker: backend + PostgreSQL
├── Dockerfile                          # Образ Django
│
├── LICENSE                             # MIT License
├── manage.py                           # Точка входа Django
├── pyproject.toml                      # Зависимости (uv)
├── README.md                           # Документация проекта
├── shortener.postman_collection.json   # Коллекция Postman
└── uv.lock                             # Lock-файл зависимостей (uv)
```
---

## Запуск проекта через Docker

**1. Запуск контейнеров**
```
docker compose up -d --build
```
В docker-compose.yml поднимаются два сервиса:

shortener_web — Django-приложение

shortener_db — PostgreSQL 15 + volume postgres_data

**2. Проверить, что контейнеры запущены**
```
docker compose ps
```
**3. Применить миграции**

(контейнеры должны быть запущены)

Можно любым вариантом

Вариант 1 — через compose (предпочтительно):
```
docker compose exec web uv run python manage.py migrate
```
Вариант 2 — через docker exec:
```
docker exec -it shortener_web uv run python manage.py migrate
```
**4. Создать суперпользователя**

Вариант 1 — через compose (рекомендуемый):
```
docker compose exec web uv run python manage.py createsuperuser
```
Вариант 2 — через docker exec:
```
docker exec -it shortener_web uv run python manage.py createsuperuser
```
**5. Просмотреть логи**

Логи веб-контейнера:
```
docker compose logs web
```
или
```
docker logs shortener_web
```
**6. Отключить и остановить контейнеры**
```
docker compose down
```
---

## Адреса

После запуска приложение будет доступно по адресу:

- Главная страница: http://localhost:8000/
- Страница со всеми ссылками (Frontend): http://localhost:8000/my/links/
- Админка: http://localhost:8000/admin/
- Swagger UI: http://localhost:8000/api/docs/
- OpenAPI-схема: http://localhost:8000/api/schema/

---

## Авторизация (JWT)

**Получить пару токенов:**
```
POST /api/v1/auth/token/
Content-Type: application/json

{
    "username": "admin",
    "password": "admin_password"
}
```

**Обновить access-токен:**
```
POST /api/v1/auth/token/refresh/

{
    "refresh": "<refresh_token_here>"
}
```

## Публичное сокращение ссылки

**Эндпоинт:**

- POST /api/v1/shorten/ — доступен без авторизации

**Тело запроса:**
```
{
  "original_url": "https://example.com/very/long/url",
  "title": "My short URL"
}
```

**Пример ответа:**
```
200 OK
{
  "id": 1,
  "short_code": "5y8L09A",
  "short_url": "/r/5y8L09A/",
  "original_url": "https://example.com/very/long/url",
  "title": "My short URL",
  "clicks_count": 0,
  "is_alive": true,
  "created_at": "2025-11-15T14:31:00Z"
}
```

## Редирект по короткой ссылке

**Эндпоинт:**

- GET /r/<short_code>/

**Пример:**

GET http://localhost:8000/r/5y8L09A/

- Ищется объект Link по short_code
- В базу добавляется событие ClickEvent с IP, браузером, ОС
- Пользователь перенаправляется на original_url

## Работа со ссылками (требуется JWT)

**Список ссылок текущего пользователя:**

- GET /api/v1/links/

Требуется авторизация:
```
Authorization: Bearer <token>
```

**Создать ссылку под своим пользователем:**

- POST /api/v1/links/
```
{
  "original_url": "https://example.com",
  "title": "Example",
  "category": 1,
  "tags": [1, 2]
}
```

**Детали / изменение / удаление конкретной ссылки:**

- GET /api/v1/links/{id}/
- PATCH /api/v1/links/{id}/
- DELETE /api/v1/links/{id}/

**Категории и теги (доступ по JWT)**

- GET /api/v1/categories/
- GET /api/v1/tags/

Используются для группировки ссылок. Создавать/редактировать их удобно через админку.